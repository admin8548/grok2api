...
def normalize_console_tools_for_model(
    console_model: str | None,
    tools: list[dict[str, Any]] | None,
) -> list[dict[str, Any]]:
    normalized = ensure_console_web_search(tools)
    if "multi-agent" not in (console_model or ""):
        return normalized

    # console multi-agent endpoints reject local/Codex function tools.  Keep
    # search as the only upstream tool class so local function definitions are
    # not blindly forwarded into a model family that cannot use them.
    search_tools = [
        tool for tool in normalized
        if str(tool.get("type") or "").strip() in _SEARCH_TOOL_TYPES
    ]
    return search_tools or [{"type": "web_search"}]
...
def build_console_chat_payload(
    *,
    ...
    normalized_input, instructions = openai_messages_to_console_input(messages)
    normalized_tools = normalize_console_tools_for_model(console_model, tools)
...
def build_console_responses_payload(
    *,
    ...
    normalized_tools = normalize_console_tools_for_model(console_model, tools)
    # Do not reduce Codex's normal function-tool set for non multi-agent models.
    # Matrix testing showed the full function-tool bundle is accepted there.
    # Non-function/namespace tools are filtered in _normalize_console_tool because
    # console /v1/responses rejects that schema with 422, but valid function
    # tools must be preserved for supported model families so Codex can inspect
...
__all__ = [
    ...
    "normalize_console_tools_for_model",
