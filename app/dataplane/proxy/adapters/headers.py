...
def build_console_headers(
    ...
    *,
    ...
    Console uses the same SSO cookie pair, but has a different origin/referer
    and expects an anonymous bearer token plus x-cluster routing header.
    """
    profile = _resolve_profile(lease)
    raw_ua = profile.user_agent
    ua = _sanitize(raw_ua, field="user_agent")
    ...
    headers: dict[str, str] = {
        ...
        "Cookie": build_sso_cookie(cookie_token, lease=lease),
    ...
    headers.update(_client_hints(browser, raw_ua))
    logger.debug("console headers built: header_count={}", len(headers))
    return headers
